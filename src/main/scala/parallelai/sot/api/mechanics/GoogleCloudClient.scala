...
trait GoogleCloudClient {
  ...
  val dataflow: Dataflow = new Dataflow.Builder(httpTransport, jsonFactory, credential).setApplicationName(api.name).build

  private def credential: GoogleCredential = {
    // Authentication is provided by gcloud tool when running locally// Authentication is provided by gcloud tool when running locally
